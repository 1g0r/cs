...
namespace LoL
{
	...
	private static TResult OpenReader<TResult>(Func<DataReader> factory, Func<SqlReader, TResult> resultReader)
	{
	...
	private static int ReadInt(SqlReader reader)
	{
		...
		.Enter("Begin secure", "fooo", SecureEval)
		.Then<SqlConn>(OpenConnection)
                .With(conn => conn.GetReader())
		.Then(OpenReader)
		.Return(Read);
