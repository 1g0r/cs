...
{
	[CustomJsonObject(SupportedNames.Commands.Namespace, SupportedNames.Commands.SkipWithText)]
	internal class SkipWithText : CommandBase<IEnumerable<IElement>, IEnumerable<XElement>>
	{
		...
		protected override object Execute(ParserContext context, IEnumerable<IElement> data)
		{
			...
			return data
				.Where(x =>
				{
					...
					return textRegexes.All(r => !r.IsMatch(text));
				})
				.ToResult();
		}
