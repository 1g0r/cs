...
{
	[CustomJsonObject(SupportedNames.Commands.Namespace, SupportedNames.Commands.BreakWhen)]
	internal class BreakWhen : CommandBase<IEnumerable<string>, IEnumerable<IElement>, IEnumerable<XElement>>
	{
		...
		protected override object Execute(ParserContext context, IEnumerable<string> values)
		{
			...
			{
				return values
					.TakeWhile(x => !breakRegexes.Any(r => r.IsMatch(x)))
					.ToResult();
			}
			return null;
		}
		...
		protected override object Execute(ParserContext context, IEnumerable<IElement> elements)
		{
			...
			{
				return elements
					.TakeWhile(x => !Patterns.Any(x.ElementMatches))
					.ToResult();
			}
			return null;
		}
